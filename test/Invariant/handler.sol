...
contract Handler is Test {
    ...
    function executeTransaction(uint256 trxIdSeed,uint256 signerIndexSeed) public {
        ...
        vm.prank(selectedSigner);
        (bool success, ) = address(multiSigTimelock).call(
        abi.encodeWithSelector(multiSigTimelock.executeTransaction.selector, trxId)
        );
    ...
    function _getValidActiveTrxIdAndSigner(uint256 trxIdSeed,uint256 signerIndexSeed) internal returns (uint256,address) {
